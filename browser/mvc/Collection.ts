...
//convert-files
import {IRetrievable} from "./IRetrievable";
//convert-files
import {IPartRequest} from "./IPartRequest";

    /**
     * Collection class
     */
    export class Collection<T extends IModel> extends EventDispatcher implements IRetrievable, IModel
    {
        /**
         * Default part
         * @type {string}
        ...
        public _add(model:T):void
        {
            ...
            if(model)
            {
                ...
                model.on(require("./Model").Model.EVENT_CHANGED, this._onChange, this, model);
        ...
        public _remove(model:T):void
        {
            if(model)
            {
                model.off(require("./Model").Model.EVENT_CHANGED, this._onChange, this);
                this._triggerUpdate(model);
            }
        }
        /**
        ...
        protected _triggerUpdate(model:T):void
        {
            ...
            if(!(model instanceof EventDispatcher))
            {
                debugger;
            }
            ...
            if(input)
            {
                if(input.forEach)
                    input.forEach(function(rawModel:any):void
                    {
                        if(rawModel.__class)
                        {
                            var model:T = <any>require("./Model").Model.get(rawModel.__class);
                        ...
                        }else
                        {
                            ...
                            {
                                var model:T = <any>require("./Model").Model.get(this.getDefaultClass());
                                model.readExternal(rawModel);
                                this.push(model);
                            }else
                            {
