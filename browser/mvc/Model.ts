...
import {Strings} from "ghost/utils/Strings";
    ...
    //convert-import
import {Collection} from "browser/mvc/Collection";

    /**
     * Model class
     */
    export class Model extends EventDispatcher implements IRetrievable, IModel
    {
        /**
        ...
        public executePartRequest(name:string, params:any = null, ajaxOptions:any = null):Promise<any>
        {
            ...
            {
                var request:any = this.getPartRequest(name, params);
                request.retry = RETRY_INFINITE;
                request.url = this.getRootURL()+request.url;
                request = Objects.mergeObjects(request, ajaxOptions);
                ajax(request).then(resolve, reject);
            });
    ...
    export class ModelID extends Model
    {
        private _generateID():void
        {
            if(!this.hasID())
            {
                this.setID(Strings.getUniqueToken());
            }
        }
        public getID():string
        {
