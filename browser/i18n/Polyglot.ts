...
///<module="data"/>
	...
	export class Polyglot extends EventDispatcher
	{
		/**
		 * Polyglot instance
		 * @type {Polyglot}
		...
		*/
		...
		protected cache(): Warehouse {
			if (!this._cache) {
				this._cache = Warehouse.instance().warehouse("polyglot");
			}
			return this._cache;
		}
		...
		public retrieveUpdate(): void {
			if (this._updated) {
				...
				if (found) {
					ajax(this.getRootURL() + "translate/update", <any>
						{
		...
		public retrieveTranslationFromServer(key: string): void {
			...
			ajax(this.getRootURL() + "translate/resolve", <any>
				{
		...
		public t(key: string, options: any = null): string {
			...
			var phrase: ITranslation = this.phrases[key + "-" + Application.instance().user.type] || this.phrases[key] || options._;
			if (!phrase) {
				this.retrieveTranslationFromServer(key);
				phrase = this.phrases[key + "-" + Application.instance().user.type] || this.phrases[key];
			}
			if (!phrase) {
