...
//convert
 /*(ghost.hasClass(*/
import {hasClass} from "ghost/core/init";
//convert
 /* ghost.getClassByName(*/
import {getClassByName} from "ghost/core/init";
...
import {Navigation} from "browser/navigation/Navigation";
    ...
    if(Hardware.isWebsite() || Hardware.isIOS() || Hardware.isAndroid())
    {
    ...
    }else
    {
        ...
        {
            throw(new Error(Hardware.getOS()+" is not supported"));
        }
    }
    /**
     * Transition helper
    ...
    export class ViewTransition extends EventDispatcher
    {
        /**
         * Css function
         * @type {Function}
    ...
    export class TransitionManager extends EventDispatcher
    {
        ...
        private _link($root?:any)
        {
            ...
            if($root)
            {
                ...
                if(name)
                    Eventer.on(Eventer.PAGE_CHANGED+":"+name, this._onPageChanged, this);
                //$root.children().height($root.height());
                //$root.children().height("100%");
                if(hasClass("ghost.browser.navigation.Navigation"))
                {
                    var childname = null;
                    var $first;
                    if($root.attr("first"))
                    ...
                    if(childname)
                    {
                        getClassByName("ghost.browser.navigation.Navigation").pushPage(name,childname);
                    }
                }
